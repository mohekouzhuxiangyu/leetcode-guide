...
| 功能 | 说明 |
|---|---|
| 📥 单条 / 批量生成 | 输入链接自动生成：中文题目、算法解析、逐步动画演示、流程图、Python/Java/C++ 代码、答题模板；**系统内已有题解（共享目录或任一用户生成过）直接复用**，不重复生成、节省生成成本，但用户首次获得该题仍按标准计费（普通 1 元/题，VIP 0.1 元/题），各用户仅看到自己的题目列表 |
| 🆓 hot100 免费浏览 | 游客可查看全部 100 道热题（只读）；登录后管理自己的题目 |
| 🗂️ 分组 / 分类 / 难度 / 搜索 | 题目按算法分类、难度、自定义分组管理，支持关键词搜索；分类/难度数字随当前分组联动刷新；刷新默认选中 hot100 |
| 🎬 动画演示 | 具体示例逐步推演（数组/指针/哈希表可视化，可播放） |
| 🎥 视频讲解 | 管理员可为每道题上传视频讲解（mp4/webm/mov）或**配置外站视频链接**（B站/YouTube 等，B站直接内嵌播放），普通 / VIP 用户均可免费观看（含 hot100）；上传后题目解析页顶部展示播放器，历史列表带 🎬 标记 |
| 📋 答题模板库 | 21 类算法固定 Python 模板 + 用户自定义模板（登录后可新建 / 修改 / 删除），支持标签筛选与搜索；自定义模板会用于对应分类的代码生成 |
| 📝 题目心得 | 每道题可写 Markdown 心得（富文本预览） |
| 👤 用户系统 | 注册 / 登录 / 邮箱验证（SMTP，未配置时开发模式直出验证链接）；登录后可修改密码（其他会话自动失效） |
| 👑 管理员 | 注册时使用 `ADMIN_EMAIL`（.env）对应邮箱即成为管理员；右上角「👑 管理」可查看全部用户与权限（VIP / 邮箱验证 / 今日用量 / 捐赠金额 / 余额 / 题目数 / 分组数），一键开通或取消任意用户的 VIP、调整余额、查看捐赠明细与**网站访问统计**（总浏览量、独立 IP、近 14 天趋势、TOP IP、最近访问） |
| 👑 VIP / 计费 | 扫码捐赠 ≥1 元自助开通永久 VIP（诚信制）；生成题目从**账户余额**扣费（普通 1 元/题，VIP 0.1 元/题），扫码支付后由管理员充值余额，余额不足无法生成；每日每账号 200 题上限 |
| 💰 支持本站 | 微信/支付宝收款码自愿捐赠，含维护运维费用说明页 |
| 🌙/☀️ 主题 | 暗夜/白日两套独立 CSS，一键切换 |
| 📱 手机端 | 响应式布局：窄屏单列堆叠、标签页横滑、输入框 ≥16px 防 iOS 聚焦缩放、表格横向滚动 |
...
4. 左侧：分组 / 算法分类 / 难度筛选 + 搜索；点击题目整行切换；刷新页面默认选中「hot100」分组
5. 「🚀 批量生成」页可直接「➕ 新建分组」，生成后题目自动归入所选分组（批量目标只能是自己的分组）
6. 「📋 算法模板库」按算法找模板；登录后点「➕ 新建模板」创建自己的模板，卡片上可 ✏️ 编辑 / 🗑 删除；「💖 升级 VIP」页含计费与运维费用说明
7. 分组规则：普通用户最多 **3 个**分组，VIP 最多 **100 个**；分组可 🗑 删除（题目保留）；`hot100` 为共享分组，**不可删除、不可占用同名**
